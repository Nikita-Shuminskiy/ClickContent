...
export const navToMarket = async (payload: { user: IUserDto }) => {
    ...
    if (payload?.user?.id) {
        ...
        window.location.href = `${HOST_LINK}${params?.page ? `${params?.page}/` : ''}`
        return
...
export const setUserCookie = (payload: { user: IUserDto }) => {
    document.cookie = `user=${encodeURIComponent(JSON.stringify(payload.user))}; ${CURR_DOMAIN} max-age=315360000;  path=/; samesite=None; secure;`
}
