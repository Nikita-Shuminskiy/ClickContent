...
const createAxiosInstance = (baseURL: string): AxiosInstance => {
    ...
    instance.interceptors.response.use(
        ...
        async (error) => {
            ...
            if(errorRefresh) { // todo refresh возвращает 401
                return Promise.reject(error);
            }
            ...
            if ((error.response?.status === 401 && !originalRequest._retry) && refreshToken) {
                ...
                if (isRefreshing) {
                    try {
                        ...
                        originalRequest.headers.set(HttpHeader.Authorization, `Bearer ${token}`)
                ...
                try {
                    ...
                    const { data } = await updateRefreshToken({ refreshToken, accessToken });
                    ...
                    originalRequest.headers.set(HttpHeader.Authorization, `Bearer ${data.accessToken}`)
                    return instance(originalRequest);
                } catch (err) {
