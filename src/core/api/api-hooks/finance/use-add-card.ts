...
import {IUserDto, PaymentStatus, SuccessfulResponseDto} from '@/data-contracts'
...
const RESOLVED_PAYMENT_STATUSES = [PaymentStatus.AddCard, PaymentStatus.AddCardError, PaymentStatus.NotFound]
    ...
    return useMutation({
        ...
        onSuccess: async (data) => {
            ...
            await executePooling(
                ...
                1000,
                async (result) => {
                    if ((result as SuccessfulResponseDto).success === PaymentStatus.AddCard) {
                        closeModal()
                        openModal({
                        ...
                        user && user.balance.total && await executePooling(
                            () => refetch(),
                            (result) => (result as QueryObserverResult<IUserDto>).data.balance.total !== user.balance.total,
                            1000,
                            () => {
                            },
                            () => {
                    ...
                    if ((result as SuccessfulResponseDto).success === PaymentStatus.AddCardError || (result as SuccessfulResponseDto).success === PaymentStatus.NotFound) {
                        closeModal()
                        openModal({
