...
const PROCESSED_PASSPORT_STATUSES = [
  IPassportStatusDto.ValidationSucceeded,
  IPassportStatusDto.ValidationFailed,
];
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      await executePooling(
        () => refetch(),
        (result) =>
          PROCESSED_PASSPORT_STATUSES.includes(
            (result as QueryObserverResult<IPassportDto, unknown>).data.status,
          ),
        2000,
        async (result) => {
          const { data: passport } = result as QueryObserverResult<
            IPassportDto,
            unknown
          >;
        ...
        (error) => {
          ...
          showAlert(
            "Не удалось валидировать паспортные данные, попробуйте позже",
            "error",
          );
