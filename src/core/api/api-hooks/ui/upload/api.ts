...
import {
    IChunkCompleteDto,
    IChunkInitDto,
    IChunkUploadCompleteRequest, IChunkUploadDto,
    IChunkUploadRequest
} from "@/core/api/api-hooks/ui/upload/type.ts";

type initRequest = ApiRequest<
    'UPLOAD.INIT',
    void,
    void,
    IChunkInitDto
...
export const sendInitUpload = async () => {
    const response = await api.post<initRequest>(
        uiInstance, {
...
export const sendChunkUpload = async (data: IChunkUploadRequest) => {
    ...
    const response = await api.post<ChunkUploadRequest>(
        uiInstance, {
...
export const sendChunkCompleteUpload = async (data: IChunkUploadCompleteRequest) => {
    const formData = new FormData()
    ...
    const response = await api.post<ChunkCompleteRequest>(
        uiInstance, {
