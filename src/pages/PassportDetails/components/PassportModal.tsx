...
export const PassportModal = ({isOpen, setOpen, setStateWarningModal}) => {
    ...
    const {mutateAsync: validatePassport, isPending: isPendingValidation} = useValidationPassport()
    const {
    ...
    const {
        mutateAsync: editPassport,
    } = useEditPassport()
    const {data: user} = useGetUser()
    const {mutate: createPassportRim, isPending: isPendingCreateRim} = useCreateRimPassport()

    const isEditPassport = user?.passportType === IPassportStatusDto.Created
    ...
    const onSubmitForm = async (data) => {
        const reqData = {
            ...
            dateOfBirth: convertDateToFormat({
                dateStr: data.dateOfBirth, inputFormat: 'dd/MM/yyyy',
                outputFormat: 'yyyy-MM-dd',
            }),
            dateOfIssue: convertDateToFormat({
                dateStr: data.dateOfIssue, inputFormat: 'dd/MM/yyyy',
                outputFormat: 'yyyy-MM-dd',
            }),
        ...
        try {
            const response = isEditPassport
                ? await editPassport({data: reqData, id: dataPassport.id}) : await createPassport(reqData);
            ...
            >
                ...
                <form onSubmit={handleSubmit(onSubmitForm)}>
                    <div className='flex flex-col gap-4 mb-8'>
                        ...
                        />
                        <SelectUI control={control}
                                  options={[{value: 'Мужской', option: 'Мужской'}, {
                                      value: 'Женский',
                                      option: 'Женский'
                                  }]}
                                  name={'sex'}
