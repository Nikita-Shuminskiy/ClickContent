...
const DonatePage = ({ data, aimId }) => {
    const hasChangedNickName = useMemo(() => getHasChangeNickName(data?.nickname), [data?.nickname])
    ...
    const handlePay = async ({ donatorComment, amount }: { donatorComment?: string, amount?: string }) => {
        // if (!userCards?.length) {
        ...
        resolver: yupResolver(
            yup.object({
                ...
                amount: yup.string().required("Это обязательное поле")
                    .test('min', `Минимальная сумма ${getCorrectPrice(settings.minDonate)}`, value => parseInt(value.replace('₽', '').trim(), 10) >= settings.minDonate)
                    .test('max', `Максимальная сумма ${getCorrectPrice(settings.maxDonate)}`, value => parseInt(value.replace('₽', '').trim(), 10) <= settings.maxDonate)
            }),
        ),
        defaultValues: {
            message: '',
    ...
    return (
        <div className="max-w-[1140px] w-full m-auto flex flex-row-reverse gap-5 items-start max-md:flex-col">
            ...
            <div
                className="flex flex-col gap-7 w-full bg-[#0e0e0e] py-[60px] px-14 text-white rounded-[35px] max-xs:p-2">
                ...
                <FormProvider {...methods}>
                    ...
                    >
                        ...
                        <div className="relative mb-3">
                            <div
                                className="flex w-full items-center justify-center flex-wrap gap-4 p-3 rounded-[60px] border border-solid border-white/10 max-xs:border-none max-xs:p-0">
