...
const Steps = () => {
  const { isNotPassport } = useUserInfoContext();
    ...
    <ModalUI isOpen={isOpen} setOpen={handleCloseStepModal} maxWidth={700}>
      ...
      <ul className="flex justify-center">
        ...
        <li className="flex justify-center">
          <div className="min-w-[200px] w-full max-w-[60%]  min-h-[400px] p-6 rounded-[32px] flex flex-col gap-5 justify-between bg-[#1B1B1B] relative overflow-hidden hover:opacity-50 transition-opacity cursor-pointer max-sm:min-h-[280px]">
            <button
...
const DonateModal = ({ isOpen, setOpen, onSuccess }) => {
  ...
  const saveDonate = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (price < 39) {
      setPriceError("Минимальная сумма 39 ₽");
      return;
    }
    if (price > 600000) {
      setPriceError("Максимальная сумма 600 000 ₽");
      return;
    }
    ...
    <ModalUI isOpen={isOpen} setOpen={setOpen}>
      ...
      <form onSubmit={saveDonate}>
        ...
        <div className="flex items-center justify-between gap-4 max-xs:flex-col-reverse">
          ...
          <ButtonUI disabled={!!priceError} type="submit">
            Установить
          </ButtonUI>
        </div>
      </form>
