...
const Quicklinks = () => {
    ...
    const allLinksLayout = (
        <div
            className="w-[60%] max-lg:min-h-[300px] min-h-[813px] gap-8 bg-color-vmire max-sm:bg-transparent p-10 rounded-[32px] relative z-[2] max-lg:w-full max-sm:p-0">
            <div className="flex justify-between items-center gap-3 mb-8">
                ...
                {quicklinks?.length > 0 &&
                    selectValue.map((option) => (
                        ...
                        >
                            {option.text} {selectedOption === option.value && (isAscending ? "↑" : "↓")}
                        </button>
                    ))}
            </div>
            {isLoading ? (
                ...
                <>
                    {quicklinks?.length > 0 ? (
                        <div>
                            {/*         _0.8fr   для тип*/}
                            <div
                                className="grid grid-cols-[1fr,_1.3fr,_1.5fr,_1.7fr,1fr,_0.1fr] items-center gap-4 mb-4 max-sm:hidden">
                            ...
                            <ul className="grid gap-4 max-sm:gap-6">
                                {data?.length > 0 &&
                                    data.map((ql, i) => {
                                        const qlJsx = <QuickLinkTableView
                                            ...
                                            ql={ql}
                                        />
                                        return <FreezeWrapper key={ql.id}>
                                            <li
                                ...
                                <TabGroup>
                                    {
                                        isMobile && <TabListUI>
                                            {tabListTitles.map((title, i) => {
                                                return <TabButtonUI key={`${i}-${title}`} text={title}/>
                                            })}
                                        </TabListUI>
                                    }
                                    <TabPanels>
                                        {isMobile && <Tab.Panel className="!outline-none">{allLinksLayout}</Tab.Panel>}
                                        <TabPanel className="!outline-none">
                                            <h3 className="text-2xl font-bold mb-[16px] max-sm:text-base">
                                                Создать платную ссылку на контент {/*mt-[24px] //todo */}
                                            </h3>
                                            <QuickLinksForm
                                                setStateQuickLinksModal={setStateQuickLinksModal}
                                                edit={null}
