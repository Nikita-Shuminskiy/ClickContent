...
export const CurrencyInput = ({amount, className, label, placeholder, suffix = '₽', name}: IProps) => {
    const {control} = useFormContext()

    const handleKeyDown = (e: KeyboardEvent<HTMLInputElement>, fieldOnChange: (value: string) => void) => {
        const input = e.target as HTMLInputElement
        const cursorPosition = input.selectionStart

        if (e.key === 'Backspace' && cursorPosition !== null && cursorPosition > 0) {
