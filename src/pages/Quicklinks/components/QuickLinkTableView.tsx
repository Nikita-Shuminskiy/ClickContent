...
const QuickLinkTableView = ({
  ...
  const { isNotPassport, isNotCardsAndPassport, isFrozenUser } =
    useUserInfoContext();
        ...
        },
        {
          ...
          onClickHandle: () => {
            setAlertProps({
              ...
              text: ql.allowResell
                ? "После удаления ссылки пользователи не смогут перепродавать ваш контент, а вы лишитесь дополнительной прибыли. Удалить ссылку?"
                : "Вы уверены, что хотите удалить быструю ссылку?",
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-[1fr,_1.3fr,_0.8fr,_1.5fr,1fr,_0.2fr] items-center gap-4 pb-3 border-b border-b-white/10 max-sm:grid-cols-1">
        {isMobile ? (
            ...
            <div className="flex items-center justify-between gap-4 w-full break-all">
              <FreezeWrapper>
                ...
                >
                  ...
                  <span className="max-w-[400px] w-full overflow-hidden line-clamp-1 text-ellipsis max-sm:w-[auto] max-sm:text-right opacity-70">
