  ...
  return (
    <div className={wrapperClassName ? wrapperClassName : ""}>
            ...
            <ul className="grid gap-4">
              {lastTransactions.map((ui, i) => {
                return (
                  <li key={`${ui.id}-${i}`}>
                    <div
                      className={`grid grid-cols-[0.6fr,0.6fr,_1.2fr,_0.4fr] gap-4 items-start pb-3 border-b border-solid border-b-white/10 max-sm:grid-cols-[1fr,_0.4fr] max-sm:gap-2 max-sm:items-center ${
                        i === lastTransactions.length - 1 ? "border-b-0" : ""
                      }`}
                    >
      ...
      <div className="max-w-[220px] w-full max-sm:max-w-full hidden max-sm:block">
        <ButtonUI
          variant="border"
          className={"max-xs:!py-[20px] max-xs:text-[16px]"}
          onClick={handleReadMore}
        >
          Смотреть больше
        </ButtonUI>
