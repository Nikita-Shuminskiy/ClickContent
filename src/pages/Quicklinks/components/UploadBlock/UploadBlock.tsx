...
const UploadBlock = ({fileIds, setFileIds}) => {
        ...
        for (const {file, id} of newFiles) {
            ...
            try {
                ...
                if (success) {
                    ...
                    setFileIds((prev) => prev.map((item) => item.id === id ? {
                        ...item,
                        isLoaded: true,
                        load: false
                    } : item));
    ...
    const handleDeleteFile = (file) => setFileIds((prev) => prev.filter((item) => item.id !== file.id));
    ...
    return (
        <>
            ...
            >
                <div
                    className=" min-h-[78px] w-full custom-border-download relative overflow-hidden cursor-pointer flex justify-center items-center p-5">
                    <span className="text-[12px] text-center max-w-[100px] font-['TTFirsNeue']">Загрузить файлы</span>
                </div>
            </FileUploader>
                    ...
                    <div key={item.id}
                        ...
                        <div className="flex gap-[11px] items-center">
                            <div className="w-7 h-7 p-1 bg-[#202020] rounded-[4px] flex">
                                {item.load ? <SpinerUI/> :
                                    <Icon name={item.isLoaded ? 'fileIcon' : 'warningIcon'}
                                          className="w-full h-full"/>}
                            </div>
                            <span className="max-w-[239px] text-sm font-inter truncate">{item.name}</span>
                        </div>
                        <div className="flex gap-5 items-center">

                            {!item.isLoaded && !item.load && (
                                <Icon name={'retryLoadIcon'} className="cursor-pointer w-[16px] h-[16px]"
                                      onClick={() => handleReuploadFile(item)}/>
                            )}

                            <Icon name={'thrashIcon'} className="cursor-pointer w-[15px] h-[19px]"
