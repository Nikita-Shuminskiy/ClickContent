...
const QuickLinksInfoModal = () => {
    ...
    const {isModalOpen, closeModal, modalParams: quickLinkInfo} = useModal(ModalKey.QUICK_LINK_DETAILS)
        ...
        <>
            {quickLink && (
                <ModalUI
                    ...
                    setOpen={closeModal}
                    maxWidth={1000}
                    modalBottomClassName={'py-8 px-[60px] max-sm:px-5 max-sm:py-0 bg-[#141414] max-sm:bg-transparent rounded-[32px]'}
                    modalBottom={quickLink?.payInTransactions.length > 0 &&
                        <ModalBottom lastTransactions={quickLink?.payInTransactions} wrapperClassName="max-sm:mb-8"/>}
                >
                  ...
                  <span className="text-sm text-white/60 max-sm:text-xs">
                    Описание
                                    ...
                                    <span className="max-sm:text-sm break-words">{quickLink.description}</span>
                                </div>
                                <div className="flex flex-col gap-1">
                  <span className="text-sm text-white/60 max-sm:text-xs">
                    Стоимость контента
                    ...
                    <div className="flex flex-col gap-8">
                        <div className="flex flex-col gap-4">
                            <div
                                className="flex w-full items-center justify-center gap-4 p-3 rounded-[60px] border border-solid border-[#FFFFFF1A] max-xs:border-none max-xs:p-0 max-xs:flex-wrap">
                                <div
                                    className="flex items-center flex-grow justify-center gap-4 px-5 overflow-hidden max-xs:border border-solid border-[#FFFFFF1A] max-xs:pb-[10px] max-xs:pt-[14px] max-xs:px-[32px] rounded-[60px] ">
                  <span className="w-6 h-6 shrink-0 ">

                                      <Icon name={'copyIcon'} className={"text-[#FFFFFFB2]"}/>
                  </span>
                                    <span
                                        className="overflow-hidden line-clamp-1 text-ellipsis max-xs:text-[12px] font-firstNeue">
