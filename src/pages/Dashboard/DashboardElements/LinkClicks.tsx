...
const LinksClicks = ({ scrollToTop, onClickLinkHover }: LinksClicksProps) => {
  ...
  return (
    <div className="w-full p-10 justify-center rounded-[32px] flex flex-col gap-7 bg-[#141414] max-sm:p-8 max-xs:p-6">
      <div className="flex z-0 gap-3 items-center justify-between max-xs:flex-col max-xs:items-start max-xs:gap-3">
        <Link
          onClick={scrollToTop}
          to="/settings"
        ...
        <FreezeWrapper>
          ...
          >
            ...
            <span className="max-xs:max-w-[110px] max-sm:max-w-[200px] max-w-[inherit] text-center break-all text-2xl max-xs:text-sm overflow-hidden font-firstNeue text-[#bcbcbc] line-clamp-1">
              {data?.url}
            </span>
          </div>
        </FreezeWrapper>
