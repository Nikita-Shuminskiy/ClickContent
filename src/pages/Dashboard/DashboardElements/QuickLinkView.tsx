...
const QuickLinkView = ({
    ...
    return (
        <div
            className={`p-6 rounded-[32px] max-xs:rounded-2xl  ${ui.errorText ? 'border-2 border-[red] rounded-[20px]' : ''} ${
            ...
            <div className="flex items-center justify-between gap-5 mb-6 pb-5 border-b border-white/10 max-xs:gap-3">
                ...
                >
                    {isFrozenUser && (
                        ...
                        <Icon name={'snow'}                       className="w-[24px] h-[24px] object-contain flex-shrink-0"/>
                    )}
                    <h3 className="text-lg font-bold max-xs:text-sm truncate">
                        {ui.title}
                    </h3>
                    {
                        ui.errorText && <p className="text-lg  max-xs:text-sm truncate text-[red]">
                            {ui.errorText}
                        </p>
                    }
                ...
                <div className="max-xs:ml-auto">
                  ...
                  {isFrozenUser ? "Заморожена" : isResale ? "Перепродажа" : ""}
                        ...
                        <DropDownUI
                            buttonRender={
                                <span
                                    className="w-6 h-6 flex items-center justify-center rounded-[50%] p-1 flex-shrink-0 bg-white/10">
                                                <Icon name={'dotsIcon'} className="w-full h-full object-cover"/>
                </span>
                            }
                            rows={[
                                {
