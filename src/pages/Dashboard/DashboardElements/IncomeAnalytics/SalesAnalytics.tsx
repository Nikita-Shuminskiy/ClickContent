...
import AnalyticsHistoryModal
    from "@/pages/Dashboard/DashboardElements/IncomeAnalytics/Modals/AnalyticsHistoryModal.tsx";
...
const SalesAnalytics = () => {
    ...
    const {data: salesData} = useGetSales({period: params.get(PARAMS_KEYS.salesPeriod) ?? 'month'})
    const {data: analyticsHistory} = useGetFinanceHistory({period: params.get(PARAMS_KEYS.financePeriod) ?? 'month'})

    const {
        openModal: setIsOpenAnalyticsHistoryModal
    } = useModal(ModalKey.ANALYTICS_HISTORY)
    const {
        openModal: setIsOpenIncomeModal,
        ...
        <>
            <div className='w-[40%] grid gap-4 max-md:w-full'>
                <div className='w-full p-10 rounded-[32px] overflow-hidden bg-[#141414] max-sm:p-8 max-xs:p-6'>
                    <Header keyParams={'salesPeriod'} title={'Количество продаж и выплат'}
                ...
                <div
                    className='w-full p-10 rounded-[32px] overflow-hidden bg-[#141414] max-sm:p-8 max-xs:p-6 min-h-[300px]'>
                    <Header keyParams={'financePeriod'} title={'Аналитика доходов'}
