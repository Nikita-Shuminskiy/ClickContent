...
const Header = ({ setIncomeModal, title, keyParams }) => {
  ...
  const currSelected =
    selectValue.find((val) => val.value === getParamValue) ?? selectValue[0];
        ...
        <div className="relative mt-1">
          <ListboxButton className="w-full text-left">
            <div className="flex items-center gap-3">
              <span
                className="text-base max-xs:text-xs text-white"
                style={{ textWrap: "nowrap" } as unknown}
              >
                {currSelected.text}
              </span>
              <span className="block w-0 h-0 border-solid border-[5px] border-r-transparent border-l-transparent border-b-0 boder-t-white"></span>
            </div>
          </ListboxButton>
          <Transition
            as={Fragment}
            ...
            leaveTo="opacity-0"
          >
            <ListboxOptions className="absolute mt-1 max-h-60 w-40 right-0 origin-top-right rounded-[16px] overflow-hidden py-4 px-5 bg-[#202020] shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
