...
const Dashboard = () => {
  const { user, isNotPassport, isNotCardsAndPassport, isFrozenUser } =
    useUserInfoContext();
  ...
  const { data: quickLinks, isLoading: isLoadingQuickLinks } =
    useGetOwnerQuickLinks();
  const { mutate } = useCreateRimPassport();

  const [selectedQuicklink, setSelectedQuicklink] = useState<IQuickLinkDto>();
  ...
  const memoizedLinks = useMemo(() => {
    return quickLinks?.slice(0, 50).map((ui, i) => {
      ...
      return (
        <SwiperSlide
          className={`max-w-[435px] w-full max-sm:max-w-[85%] ${
            i === 0 ? "pl-[30px] max-sm:pl-[20px]" : ""
          }`}
          key={`${i}-${ui?.id}`}
          onClick={() => onClickSwiperSlide(ui, "Content")}
        >
          <FreezeWrapper>{qlJsx}</FreezeWrapper>
        </SwiperSlide>
      );
    });
            ...
            <div className="flex items-start gap-4 max-md:flex-col">
              <div className="w-[60%] grid gap-4 max-md:w-full">
                ...
                {/*  <LinksClicks
                                ...
                                /> // todo реффералка */}
                <div className="w-full py-10 overflow-hidden rounded-[32px] flex flex-col gap-7 bg-[#141414] max-sm:py-8 max-xs:py-6">
                  {/* быстрые ссылки */}
                  <div>
                    <div className="flex items-center justify-between gap-5 px-10 mb-5 max-sm:px-8 max-xs:px-6 max-xs:gap-3">
                      <Link
                        onClick={scrollToTop}
                        to="/quicklinks"
                  ...
                  {/* цели */}
                  <div>
                    <div className="flex items-center justify-between gap-5 px-10 max-sm:px-8 max-xs:px-6 mb-5 max-xs:gap-3"></div>
                  </div>
                  {/* /цели */}
                </div>
              </div>
