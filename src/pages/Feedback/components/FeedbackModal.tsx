                ...
                <div className="flex flex-col gap-4 mb-8">
                    <Controller
                        ...
                        render={({field: {onChange, value}}) => {
                            return <Listbox value={value} onChange={onChange} name="type">
                                <div className="relative mt-1">
                                    <ListboxButton className="w-full text-left">
                    <span
                        className="flex flex-col truncate pl-[28px] pb-[28px] pt-[28px] max-sm:pt-[18px] max-sm:pb-[18px] pr-12 text-white bg-[#1A1A1A] text-[16px] rounded-2xl">
                      <span className="text-base w-full overflow-hidden text-ellipsis">
                        {value?.text}
                      </span>
                    </span>
                                        <span
                                            className="pointer-events-none absolute top-[50%] translate-y-[-50%] right-5 w-2 h-2 border-l-2 border-b-2 border-solid border-white -rotate-45"></span>
                                    </ListboxButton>
                                    <Transition
                                        as={Fragment}
                                    ...
                                    >
                                        <ListboxOptions
                                            className="absolute mt-1 max-h-60 w-full overflow-auto rounded-2xl bg-[#202020] text-base p-5 shadow-lg ring-1 z-20 ring-black/5 focus:outline-none sm:text-sm">
                                            {selectValue.map((option) => (
                                                <ListboxOption
                                                    className={
                                                        `relative cursor-pointer select-none hover:bg-white/20 rounded-xl`
                                                    }
                                                    key={`key-${option.value}`}
                                                    value={option}
                                                >
                                                    <div
                                                        className="flex items-center gap-3 truncate text-white text-base py-5 px-3 border-solid border-b-1 border-b-white/20">
