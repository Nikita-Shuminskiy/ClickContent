            ...
            <div className=" flex flex-col gap-[10px] items-center max-sm:!width-[100%]">
                <div
                    ...
                    {cardsAbout.slice(0, 3).map((card, index) => (
                        <Card key={index} {...card} onClick={card?.isClick && onOpenTelegram}/>
                    ))}
                </div>
                <div
...
const Card = ({title, text, color, img, classNameImg, classNameContainer, animation, onClick}) => {
    return (
        <motion.div
        ...
        >
            <div className={'py-[42px] px-[35px] flex flex-col gap-[20px] items-start'}>
                <h2 className="text-[46px] max-xl:text-[32px] font-bold font-PPNeueMachina z-[9] " style={{lineHeight: '100%'}}>{title}</h2>
                <p className="text-sm font-firstNeue text-[16px] relative z-[99]">{text}</p>
                {onClick && <ButtonUI onClick={onClick}
                                      className={'!bg-[#F8F8F8] !text-black max-w-[139px] z-[9]'}>Перейти</ButtonUI>}
            </div>

            <div className={'w-[100%] flex items-end justify-end'}>
                <img className={`${classNameImg} object-contain`} src={img} alt={title}/>
            </div>
        </motion.div>
    );
};
