...
import {ActivePageEnum, links} from "@/pages/Landing/components/constants.tsx";
        ...
        <div className="z-[999] fixed bottom-0 w-full bg-[#1A1A1A99] rounded-tr-[36px] rounded-tl-[36px] ">
            ...
            <div className=" flex gap-[6px] p-[6px]  ">
                {links.map((link, index) => (
                    <div className={'flex flex-col gap-[10px] items-center justify-between w-full'}
                         onClick={() => onClick(link.to)}>
                        <Icon name={link.icon as any}
                              className={`text-[#F8F8F8] font-bold w-[24px] h-[24px] ${activePage === link.to ? "opacity-100" : "opacity-30"} `}/>
                        <span
                            key={index}
