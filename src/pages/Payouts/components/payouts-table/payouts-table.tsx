            ...
            <ul className="grid gap-4">
                {payouts.map((payout, i) => {
                    ...
                    return (
                        <li key={i}>
                            <div
                                className="grid grid-cols-[0.6fr,_1.2fr,_0.4fr] gap-4 items-start pb-3 border-b border-solid border-b-white/10 max-sm:grid-cols-[1fr,_0.4fr] max-sm:gap-2 max-sm:items-center">
                              ...
                              {formatTime(payout.date)}
                                ...
                                <div className="max-sm:order-5 max-sm:col-span-full">
                                    <p className="text-base text-white/70 max-xs:text-[10px] max-xs:leading-[1.4]">
                                        {!payloadToObj?.pan
                                            ? 'Нет данных по карте'
                                            : `Выплата ${formatCardByPan(payloadToObj?.pan).toLocaleLowerCase()}`
                                        }
                                    </p>
                                </div>
