...
export const IncomingPaymentsTable: FunctionComponent<IProps> = memo(
  ({ finances }) => {
    ...
    return (
      <>
        <div className="grid grid-cols-[0.6fr,0.5fr,_0.5fr,_0.4fr,0.5fr,0.2fr] gap-4 mb-4 pb-3 max-sm:hidden">
        ...
        <ul className="grid gap-4">
          {finances.map((payment, i) => {
            return (
              <li
                key={`${payment.quicklinkId}__${i}`}
                onClick={(event) => openPaymentInfo(event, payment)}
                className="cursor-pointer"
              >
                <div className="grid grid-cols-[0.6fr,0.5fr,_0.5fr,_0.4fr,0.5fr,0.2fr] gap-4 items-start pb-3 border-b border-solid border-b-white/10 max-sm:grid-cols-1 max-sm:gap-2 max-sm:items-center">
