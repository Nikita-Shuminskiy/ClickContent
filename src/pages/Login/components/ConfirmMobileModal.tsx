...
const ConfirmMobileModal = ({
  ...
  const { mutate: smsVerification, isPending } = useAuthBySmsVerify({
    onError,
    isFromPayment,
    isFromMarket,
    setStateConfirmMobileIDModall,
  ...
  return (
    ...
    >
      <div className="mb-6 flex flex-col">
        ...
        <button
          className="text-[#874AB0] self-center text-sm"
      ...
      {isPending && (
        <div className="absolute top-1/2 left-1/2 z-10 bg-white/5 -translate-x-1/2 -translate-y-1/2 w-full h-full rounded-[32px]">
          ...
          <>
            {slots.map((slot, i) => (
              <FormInputUI
                ...
                label=""
                name=""
                className={`w-full max-w-[52px] max-h-[52px] !caret-white !pr-0 !pt-[24px] font-firstNeue mx-auto self-center ${
                  error ? "border-[#DE3452] border" : ""
                }`}
      ...
      ></OTPInput>
      {error && (
        <p className="text-center text-[#DE3452] mb-6 font-manrope text-xs max-w-[350px] max-xs:max-w-full mx-auto">
          {error?.info?.Message}
        </p>
      )}
      <div className="flex justify-between items-center max-sm:flex-col gap-4">
        <ButtonUI
          className={
            "max-sm:text-[16px] max-w-[229px] max-sm:max-w-full max-sm:!py-[16px] font-firstNeue"
          }
          variant="border"
          type="button"
          onClick={() => {
        ...
        {tick != 0 ? (
          <p className="font-firstNeue text-white/30 text-center text-sm">
            Отправить код повторно через <br /> {tick} сек
          </p>
        ) : (
          <ButtonUI
