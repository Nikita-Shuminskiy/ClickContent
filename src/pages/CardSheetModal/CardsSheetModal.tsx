...
export const CardsSheetModal = () => {
  ...
  const onWithdraw = async () => {
    ...
    const remainingAmount =
      dataLimits?.maxWithdrawAmountPerCard - selectedCard.payout;

    const remainingOperations =
      dataLimits?.maxWithdrawOperationsPerCard - selectedCard.operations;

    if (remainingAmount <= 0 || remainingOperations <= 0)
      return alert("Вывод по этой карте недоступен: превышен лимит.");
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center">
        <span className="block text-[18px] font-bold text-center mb-[4px]">
          {modalParams?.title}
        </span>
        ...
        <div className="w-full flex flex-col gap-[12px]">
          {sortedCards.map((card) => {
            return (
              <CardView
                isChosen={selectedCard?.id === card.id}
                onChoseCard={onChoseCard}
                card={card}
                key={card.id}
              />
            );
        ...
        <div className={"mt-[12px] w-full"}>
          <ButtonUI
            variant="border"
            className={
              " !py-[32px] max-xs:!py-[12px] max-xs:!text-xs text-[16px] "
            }
        ...
        <div
          className={"flex flex-row items-center gap-[10px] w-full mt-[32px]"}
        >
          <ButtonUI
            variant="border"
            className={
