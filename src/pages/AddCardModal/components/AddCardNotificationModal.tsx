...
export const AddCardNotificationModal = () => {
  const { isModalOpen, closeModal, modalParams } = useModal(
    ModalKey.ADD_CARD_NOTIFICATION,
  );
    ...
    <ModalUI isOpen={isModalOpen} setOpen={closeModal}>
      ...
      <div className="flex flex-col items-center">
        <div className="w-14 h-14 mb-8 mx-auto">
          <Icon
            name={modalParams?.icon}
            className="w-full h-full object-contain"
          />
        </div>
        <span className="block text-5xl font-bold text-center mb-4 max-sm:mb-2 max-md:text-4xl max-sm:text-3xl max-xs:text-2xl">
          Добавление карты
        </span>
