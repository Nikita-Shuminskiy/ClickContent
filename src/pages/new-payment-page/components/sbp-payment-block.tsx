...
export const SBPPayment: FunctionComponent<IProps> = memo(( { data, isPayRejected , isDataPending} ) => {
    const { showAlert, hideAlert } = useAlert()
    const { quickLinkId } = useParams<{ quickLinkId?: string }>()

    const { mutateAsync: pay, data: newQrData, isPending } = usePayForQuickLink()
        ...
        if (timeLeft > 0) {
            ...
            showAlert(
                <PayErrorAlert
                    ...
                    action={
                        <div className='text-[14px] leading-[19px] font-bold text-[#874AB0] cursor-pointer'
    ...
    return (
        <div
            className='max-w-[478px] !w-full pr-[82px] pt-[10px] pb-[16px] border-r border-white/10 max-lg:pr-[34px] max-lg:py-0 max-sm:pr-[29px] max-sm:pt-0 max-xs:border-t max-xs:pt-[24px] max-xs:border-white/10 max-xs:border-r-0 max-xs:px-[32px] mobile:hidden'>
            <div className='h-full w-full flex flex-col gap-6 items-center max-sm:gap-4 max-xs:gap-3'>
                ...
                <div className='px-[10px] max-lg:px-[51px] max-sm:px-[22px]'>
                    <div
                        className='w-[376px] h-[376px] bg-gradient-to-br from-[#F0826C] via-[#874AB0] to-[#863AFF] rounded-[56px] p-6 max-lg:w-[220px] max-lg:h-[220px] max-lg:p-4 max-lg:rounded-[41px] max-xs:w-[192px] max-xs:h-[192px]'>
                        <div
                            className={ `w-full h-full p-6  bg-white rounded-[36px] max-lg:p-4 max-lg:rounded-[26px] ${ isQrNotAllowed && 'blur-[7px]' }` }>
                            <QRCode value={ qr } id="qr-code-sbp" className="custom-qr-code"/>
                        </div>
                ...
                { isTimeUp ?
                    <p className='text-center font-steppe text-[12px] leading-[14.4px] text-white max-sm:text-[10px] max-sm:leading-3'>
                        ...
                        нужно <span className='inline-block underline cursor-pointer' onClick={ refreshSbpQr }>обновить QR-код</span>
                    </p>
                ...
                <div
                    className='py-3 px-6 bg-[#FFFFFF0D] rounded-[50px] w-full max-sm:rounded-[32px] max-xs:mt-[4px]   max-sm:text-[10px] max-sm:leading-3'>
                    <p className=' m-auto text-center font-steppe text-[12px] leading-[14.4px] text-white max-w-[300px] max-lg:max-w-[274px] max-sm:max-w-[188px] max-sm:text-[10px] max-sm:leading-[12px] max-xs:max-w-[310px] '>
                        ...
                        в приложении вашего банка, либо штатной
                        камерой телефона
                    </p>
                </div>
