...
import { IQuickLinkDto, PaymentStatus, SuccessfulResponseDto } from "@/data-contracts";
...
//TODO СБП убрано - может временно - пока не удаляем
...
export const useQuickLinkPayCheck: UseQuickLinkPayCheck = () => {
    ...
    const [ isQuickLinkDontPaid, setIsQuickLinkDontPaid ] = useState<boolean>(false)
    ...
    const { mutateAsync: checkPaymentStatus, isPending, data: checkPaymentData } = usePaymentStatusCheck();

    // const { mutateAsync: pay, data: SBPQrData, isPending: isPayPending, error: payError } = usePayForQuickLink()
    ...
    const isDataPending = isPending || quickLinkLoading
    // || isPayPending
    ...
    useEffect(() => {
        ...
        const checkAndRedirect = async () => {
            ...
            if (paidQuickLink) {
                ...
                const result = await checkPaymentStatus({ paymentId: paidQuickLink.paymentId });
