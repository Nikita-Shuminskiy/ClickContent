...
const PaymentPage = () => {
    ...
    const { isDataPending, SBPQrData, quickLink, checkPaymentData, payError } = useQuickLinkPayCheck()

    const isPayRejected = checkPaymentData?.success === PaymentStatus.PayInError || !!payError


    return (
        <main
            className='relative min-h-screen bg-[#0E0E0E] flex items-center justify-center px-[50px] max-sm:pl-[67px] max-sm:pr-8 max-xs:px-0 max-xs:py-[32px] mobile:pt-[68px]'>
            <BackgroundCircle
                className='top-[-255px] left-[845px] max-lg:top-[-255px] max-lg:left-[578px] mobile:top-[-223px] mobile:left-[160px]'/>
            <div
                className='h-full flex items-center gap-[82px] max-lg:gap-[55px] max-sm:gap-[29px] max-xs:flex-col-reverse max-xs:items-center max-xs:gap-6 '>

                <SBPPayment data={ SBPQrData } isPayRejected={ isPayRejected } isDataPending={ isDataPending }/>
                <OtherPaymentMethods
                    data={ quickLink }
