...
export const PayModal = () => {
  ...
  const { isModalOpen, modalParams, closeModal } = useModal(
    ModalKey.PAY_WITH_CARD,
  );
  ...
  useEffect(() => {
    ...
    const pollingInterval = setInterval(async () => {
      try {
        ...
        navigate(
          `/authorize?status=fail&error=${
            err?.info?.error || "Неизвестная ошибка"
          }`,
        );
        handleCloseModal();
      }
  ...
  return (
    <ModalUI isOpen={isModalOpen} setOpen={handleCloseModal}>
      <div className="mb-8">
        <CurrentTime />
      </div>
      {dataPay?.url && (
        <iframe src={dataPay?.url} style={{ width: "100%", height: "600px" }} />
      )}
    </ModalUI>
  );
};
