...
const ListBoxUI = ({onChange, value, placeholder, options}) => {
    return (
        <Listbox value={value ? value : null} onChange={onChange}>
            {({open}) => (
                ...
                <span
                  ...
                  <span
                      className={`${value ? 'text-white' : 'text-[#5f5f5f]'} truncate font-firstNeue`}>{value ?? placeholder}</span>
                </span>
                        <span
                ...
                >
                  <ListboxOptions
                      className='absolute overflow-auto w-full mt-1 max-h-60 rounded-2xl bg-[#1A1A1A] text-base shadow-lg ring-1 z-20 ring-black/5 focus:outline-none sm:text-sm'>
                    {options.map((option, i) => (
                        <ListboxOption
                            className={'pl-5 pt-2 first:pt-3 pb-2 hover:bg-white/20 cursor-pointer font-firstNeue'}
