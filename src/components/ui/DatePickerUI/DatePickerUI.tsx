...
type DatePickerUIProps = ReactDatePickerProps & {
    fieldError?: boolean
    iconLayout?: JSX.Element
    label?: string
}
const DatePickerUI = ({onChange, onBlur = null, fieldError, iconLayout = null, className,label, ...rest }: DatePickerUIProps) => {
  const {selected = '', placeholderText = ''} = { ...rest }
    ...
    const isActiveDatePickerClassName = (!!selected || isFocused) ? 'datePickerUI_active' : ''
    return (
        <div className={`w-full datePickerUI ${isActiveDatePickerClassName}`}>
            <DatePicker
                className={`bg-[#1A1A1A] ${fieldError ? "datapicker-input-error" : ""} ${className ?? ''}`}
                onChange={onChange}
                ...
                {...rest}
            />
            <label htmlFor={'picker'}>{(!!selected || isFocused) ? label : placeholderText}</label>
    </div>
    );
}
