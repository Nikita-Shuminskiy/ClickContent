...
export const AuthorizedUserLayout = ({ isLanding }) => {
  ...
  const { isOpenMobileMenu, setIsOpenMobileMenu, toggleMobileMenu } =
    useUserInfoContext();

  const currentLinks =
    (isLanding && isAuth) || !isAuth
      ? landingHeaderLinks
      : authorizedHeaderLinks;
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-5 relative ">
        ...
        <button
          className={`burgerBtnUI ${
            isOpenMobileMenu ? "burgerBtnUI--active" : ""
          }`}
      ...
      >
        <div className="flex flex-col gap-2 mb-6">
          ...
          <div className="flex justify-center">
            ...
            >
              <Icon
                name={"editIcon"}
                className="w-4 h-4 opacity-50 flex-shrink-0 object-cover"
              />
              Редактировать
            </button>
          </div>
        ...
        <div className="text-center">
          <button onClick={doLogout} className="text-base text-[#5F5F5F]">
            Выход
          </button>
        </div>
      </ModalUI>
