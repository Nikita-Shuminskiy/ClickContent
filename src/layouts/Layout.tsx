...
export default function Layout({children, isLanding}: { children: React.ReactNode, isLanding?: boolean }) {
    const location = useLocation();

    const hideHeaderFooterRoutes = ["/authorize", "/p", "/receipt", '/go',];
    const shouldHideHeaderFooter = hideHeaderFooterRoutes.some(route =>
        location.pathname.includes(route) && location.pathname !== "/payouts" && location.pathname !== "/policy"
    );
