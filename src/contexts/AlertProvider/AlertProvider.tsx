...
interface AlertContextProps {
    showAlert: (message: string | ReactNode, type: AlertType, duration?: number) => void;
    hideAlert: () => void;
}
...
export const AlertProvider = ({children}) => {
    ...
    const showAlert = (message: string | ReactNode, type: AlertType, duration = 3000) => {
        ...
        <AlertContext.Provider value={{showAlert, hideAlert}}>
            ...
            {alert && (
                <div className='w-full flex justify-center fixed top-10 left-2/4 -translate-x-2/4 z-[100000] px-[15px]'>
                    <div
                        className='flex items-center gap-[12px] px-[20px] py-[16px] bg-[#1A1A1A] rounded-[14px] shadow-lg w-[375px] max-sm:w-full justify-between'
                    >
                        <div className={'flex flex-row items-center gap-[12px]'}>
                            {statusIcon[alert.type]}
                            <div className={'font-manrope text-[14px]'}> {alert.message}</div>
                        </div>
                        <button className='text-[24px] text-[#575859] relative top-[2px]' onClick={hideAlert}>
                            &times;
                        </button>
                    </div>
                </div>
