...
export const ModalProvider = ({ children }: { children: ReactNode }) => {
  const [openModals, setOpenModals] = useState<Map<ModalKey, ModalState>>(
    new Map(),
  );

  const openModal = useCallback(
    (modalKey: ModalKey, params?: ModalParams) => {
      setOpenModals((prev) =>
        new Map(prev).set(modalKey, { isOpen: true, params }),
      );
    },
    [setOpenModals],
  );
  ...
  const isModalOpen = (modalKey: ModalKey) =>
    openModals.get(modalKey)?.isOpen || false;
  const getModalParams = (modalKey: ModalKey) =>
    openModals.get(modalKey)?.params;
