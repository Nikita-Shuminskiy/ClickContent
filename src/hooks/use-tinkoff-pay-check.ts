...
const PROCESSED_STATUSES = [ PaymentStatus.PayIn, PaymentStatus.PayInError, PaymentStatus.NotFound ]

export const useTinkoffPayCheck = ( setInitialLoading: ( value: boolean ) => void ) => {
    const { status } = useParams<{ status: string }>()

    const payments = StorageService.getPayments()
    const currentActivePayment = payments && payments.filter(p => p.isActive)[0]

    const { mutateAsync: paymentCheck, data: paymentStatus } = usePaymentStatusCheck()
            ...
            if (status === 'success') {
                ...
                if (currentActivePayment?.paymentId) {
                    await executePooling(
                        ...
                        result => PROCESSED_STATUSES.includes((result as SuccessfulResponseDto).success ),
                        1000,
                        ( result ) => {
                            if ((result as SuccessfulResponseDto).success === PaymentStatus.PayIn) {
                                navigate(`/receipt?id=${ currentActivePayment?.id }`);
                            }
                            if ((result as SuccessfulResponseDto).success === PaymentStatus.PayInError || (result as SuccessfulResponseDto).success === PaymentStatus.NotFound) {
